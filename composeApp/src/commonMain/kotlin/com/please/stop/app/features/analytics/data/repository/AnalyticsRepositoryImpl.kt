...
class AnalyticsRepositoryImpl(
    ...
    override fun observeAnalyticsData(): Flow<AnalyticsData> {
        val currentMonthRange = currentMonthMillisRange()
        val today = localDateToday()
        val monthRanges = buildMonthRanges(today.year, today.monthNumber)

        val daysInMonth = LocalDate(today.year, today.monthNumber, 1).lengthOfMonth()

        return combine(
            categoryDao.observeAll(),
            expenseDao.observeSpendingByCategory(currentMonthRange.fromMillis, currentMonthRange.toMillis),
            expenseDao.observeTotalSpending(currentMonthRange.fromMillis, currentMonthRange.toMillis),
            expenseDao.observeExpensesInRange(currentMonthRange.fromMillis, currentMonthRange.toMillis),
            ...
            val monthlyTotals = buildMonthlyTotals(monthRanges, today.year, today.monthNumber)
